...
from test_framework.mininode import jagoancoinhash
...
def jagoancoinhash_helper(b):
    return encode(jagoancoinhash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def setup_nodes(self):
        ...
        # Check that jagoancoin has been built with ZMQ enabled
    ...
    def _zmq_test(self):
        ...
        assert_equal(genhashes[0], jagoancoinhash_helper(body[:80]))
        ...
        for x in range(n * 4):
            ...
            if topic == b"hashblock":
                ...
                blockcount += 1
            if topic == b"rawblock":
                zmqRawHashed.append(jagoancoinhash_helper(body[:80]))
                msgSequence = struct.unpack('<I', msg[-1])[-1]
