...
class BitcoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use jagoancoin-cli instead of RPC for all commands")
        parser.add_option("--jagoancoind-arg", dest="dashd_extra_args", default=[], type='string', action='append',
                          help="Pass extra args to all jagoancoind instances")
        ...
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/jagoancoind' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/jagoancoin-cli' + config["environment"]["EXEEXT"])
...
def skip_if_no_bitcoind_zmq(test_instance):
    """Skip the running test if jagoancoind has not been compiled with zmq support."""
    config = configparser.ConfigParser()
    config.read_file(open(test_instance.options.configfile))

    if not config["components"].getboolean("ENABLE_ZMQ"):
        raise SkipTest("jagoancoind has not been built with zmq enabled.")
