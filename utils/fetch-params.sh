...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/JagoancoinParams"
else
    PARAMS_DIR="$HOME/.jagoancoin-params"
fi
...
DOWNLOAD_URL="https://github.com/jagoanpilot/jagoancoin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with JGC_DISABLE_SOMETHING=1
JGC_DISABLE_WGET="${JGC_DISABLE_WGET:-}"
#JGC_DISABLE_IPFS="${JGC_DISABLE_IPFS:-}"
JGC_DISABLE_CURL="${JGC_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$JGC_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$JGC_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$JGC_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Jagoancoin parameters!
...
This script will fetch the Jagoancoin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Jagoancoin SNARK parameters. Note that it is
