...
{
    return QCoreApplication::translate("jagoancoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in jagoancoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in jagoancoin.qrc)
...
/** Class encapsulating Jagoancoin Core startup and shutdown.
...
/** Main Jagoancoin application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // jagoancoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Jagoancoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Jagoancoin Core - \""), false, OptionsCategory::GUI);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(jagoancoin);
    Q_INIT_RESOURCE(jagoancoin_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse jagoancoin.conf
    ...
    // jagoancoin: links repeatedly have their payment requests routed to this process:
