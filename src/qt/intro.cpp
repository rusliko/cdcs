    ...
    signalled(false)
{
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Jagoancoin block chain.").arg(tr(PACKAGE_NAME)) + " " +
...
bool Intro::pickDataDirectory(interfaces::Node& node)
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || dataDirDefaultCurrent != dataDirDefaultSettings)
    {
        ...
        intro.setWindowIcon(QIcon(":icons/jagoancoin"));
     ...
     * override -datadir in the jagoancoin.conf file in the default data directory
     * (to be consistent with jagoancoind behavior)
     */
